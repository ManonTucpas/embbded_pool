...
void init_rgb()
{
    ...
    TCCR0A |= (1 << WGM01) | (1 << WGM00);   // Fast PWM avec TOP = 255 - p.115
    // Prescaler est /64 pour avoir une frequence PWM ~976Hz donc le cycle fait ~1ms, on percoit pas le scintillement
...
void set_rgb(uint8_t r, uint8_t g, uint8_t b)
{
    ...
    // Quand le timer atteint la valeur OCRxn la LED s'eteint
    OCR0B = r;
    OCR0A = g;
    OCR2B = b;
}